...
#
# bash 5.0 doesn't seem to like the older `function foo` syntax that ksh needs.
# To make this file work for ksh, run `alias local=typeset`
# and then source this file with something like
#    source /dev/stdin < <(sed 's/^\(.*\)()$/function \1/' /path/to/path_functions.sh)

### exists
# Tell if a runnable program exists with the given name.
exists()
{
    # command is a shell builtin; which is an external binary
...
# Allows adding nonexistent paths if -n is given.
# Set a different path separator from ":" with -sSEP.
add_to_path()
{
    local TO_END=
    local FORCE=
    ...
    while [ "x$1" != "x" ] ; do
        ...
        if ! is_in_path "$PATHVAR" "$1" "$SEP" && \
                ! is_in_path ADD PATHS "$1" "$SEP" ; then
            if ! [ -d "$1" ] && [ -z "$NONEXIST" ] ; then
                if [ -z "$QUIET" ] ; then
                    echo "add_to_path: $1 not added to $PATHVAR (not found)" 1>&2
                fi
                shift
                continue
            fi
